...
interface SidebarInventaireAddStock {
  open: boolean
  toggle: () => void
  onChange: () => void
...
const schema = yup.object().shape({
  ...
  types: yup.string().required(() => 'Le champ type de mouvement est obligatoire'),
...
const SidebarInventaireAddEntree = (props: SidebarInventaireAddStock) => {
  // ** Props
  const { open, toggle, onChange, onSuccess, currentEntree, produits } = props
  ...
  const onSubmit = async (data: EntreeData) => {
    const entreeR1Service = new InventaireService()
    setSend(true)
    ...
    if (id === -1) {
      const result = await entreeR1Service.createEntreeInventaire(sendData)
      setSend(false)
      ...
      entreeR1Service
        .updateEntreeInventaire({ ...sendData, id }, id)
        .then(rep => {
          setSend(false)
          if (rep) {
            onChange()
  ...
  useEffect(() => {
    reset({
      ...
      types: currentEntree && currentEntree?.types !== undefined ? currentEntree.types : '',
  ...
  return (
    ...
    >
      <Header>
        <Typography variant='h6'>{id === -1 ? 'MISE A JOUR STOCK' : 'MODIFICATION DE MISE A JOUR STOCK'}</Typography>
        <IconButton
          size='small'
          onClick={handleClose}
          sx={{
          ...
          <Controller
            ...
            render={({ field: { value, onChange } }) => (
              <TextField
                ...
                {...(errors.types && { helperText: errors.types.message })}
              >
                <MenuItem value=''>Sélectionnez un type</MenuItem>
                <MenuItem value='ADD'>ENTREE</MenuItem>
...
export default SidebarInventaireAddEntree
