...
interface SidebarAddModelType {
  open: boolean
  toggle: () => void
  onChange: () => void
  onSuccess: (data: any) => void
  currentModel: null | Model
}
...
const SidebarAddModel = (props: SidebarAddModelType) => {
  // ** Props
  const { open, toggle, onChange, onSuccess, currentModel } = props
  ...
  const onSubmit = async (data: ModelData) => {
    ...
    if (id === -1) {
      const result = await modelService.createModel(sendData)
      setSend(false)
  ...
  return (
    ...
    >
      <Header>
        <Typography variant='h5'>{id === -1 ? 'Ajout de model' : 'Modification de model'}</Typography>
        <IconButton
          size='small'
          onClick={handleClose}
          sx={{
